...
**גרסה:** 3.3.0 (Smart Stock Selection)
...
|------|-------|-----------------|
| V3.3 | 25/11/2025 | Smart Stock Selection - מערכת ניקוד רב-פקטורית, סקאנר משופר, קונצנזוס אסטרטגיות |
| V3.2 | 25/11/2025 | Realistic Day Trading - SL 1x ATR, TP 1.5x ATR |
| V3.1 | 25/11/2025 | ATR Stability - סף מינימום, ולידציה, לוגים משופרים |
| V3.0 | 25/11/2025 | Multi-Strategy Engine + ATR Position Sizing |
...
4.  **בחירה חכמה:** מערכת ניקוד לבחירת TOP 5 מניות (✅ V3.3).
5.  **למידה:** איסוף נתונים ושיפור אסטרטגיות מבוסס AI (🔄 בתהליך).
...
│                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │   Scanner    │  │   Scorer     │  │   Ranker     │      │
│  │  (Enhanced)  │──▶│ (5 Factors) │──▶│ (TOP 5)     │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
│         │                                    │              │
│         ▼                                    ▼              │
│  ┌──────────────────────────────────────────────────┐      │
│  │              5 Strategies Engine                 │      │
│  │  MA Cross | Bollinger | ORB | VWAP | Volume     │      │
│  └──────────────────────────────────────────────────┘      │
│                          │                                  │
│                          ▼ (2+ Consensus Required)          │
│  ┌──────────────────────────────────────────────────┐      │
│  │           Trade Manager (V3.2)                   │      │
│  │    ATR Position Sizing | SL 1x | TP 1.5x        │      │
│  └──────────────────────────────────────────────────┘      │
│                          │                                  │
│                          ▼                                  │
│  ┌──────────────────────────────────────────────────┐      │
...
*   **`src/scanner`**: סורק משופר עם 3 סוגי סריקה (Gap Up, Hot Volume, Most Active).
*   **`src/scoring`**: מערכת ניקוד רב-פקטורית לבחירת TOP 5 מניות.
...
## 3. ⚙️ קונפיגורציה נוכחית (V3.3)

### 🎯 מערכת ניקוד מניות (Stock Scorer) - חדש!
כל מניה מקבלת ניקוד 0-100 לפי 5 פקטורים:

| פקטור | ניקוד מקסימלי | תיאור |
|-------|---------------|-------|
| Gap % | 20 נקודות | פער מסגירה אתמול (מינימום 1%, מצוין 5%+) |
| RVOL | 25 נקודות | נפח יחסי לממוצע (מינימום 1.5x, מצוין 5x+) |
| ATR % | 15 נקודות | תנודתיות יומית (מינימום 2%, מצוין 6%+) |
| Momentum | 20 נקודות | מומנטום 5-bar (מינימום 0.5%, מצוין 3%+) |
| Signals | 20 נקודות | הסכמת אסטרטגיות (4 נקודות לכל אסטרטגיה) |

**סף מינימום:** 25 נקודות לכניסה לדירוג
**נדרש לקנייה:** קונצנזוס של 2+ אסטרטגיות

### 📊 סקאנר משופר (Enhanced Scanner)
שלוש סריקות במקביל:

| סריקה | קוד IB | תיאור | תוצאות |
|-------|--------|-------|--------|
| Gap Up | TOP_OPEN_PERC_GAIN | מניות שפתחו בפער חיובי | 15 |
| Hot Volume | HOT_BY_VOLUME | נפח יחסי גבוה | 15 |
| Most Active | MOST_ACTIVE | הנסחרות ביותר | 10 |

**תוצאה:** ~36 מניות ייחודיות ← מנותחות ← TOP 5 נבחרות

### ניהול סיכונים (Risk Management) - V3.2
...
*   **Take Profit:** 1.5x ATR (יחס סיכון/סיכוי 1:1.5 - ריאליסטי ל-Day Trading).
*   **Stop Loss:** 1x ATR.
